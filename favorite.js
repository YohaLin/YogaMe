...
const friends = JSON.parse(localStorage.getItem('favoriteLists')) || [];
console.log(friends)
let filteredCard = [];
...
function renderFriendsList(file) {
  ...
  <div class="bg-white m-2 pt-4 d-flex flex-column align-items-center rounded-4" style="width: 15rem;">
      ...
      <div class="card-body d-flex flex-column align-items-center">
        ...
        ${item.age} | ${item.gender} | <i class="fa-solid fa-heart text-danger" data-id="${item.id}"></i>
        </p>
      </div>
    </div>
  `;
    ...
    <li class="list-group-item d-flex justify-content-between">
        ...
        <div>
          ...
          <i class="fa-solid fa-heart text-danger" data-id="${item.id}"></i>
        </div>
    </li>
  `;
    });
...
function removeFromFavorite(id) {
  // 這裡加上兩個條件控制：(１)一旦收藏清單是空的，就結束這個函式(用return的方式結束即可～)。
  if (!friends || !friends.length) return

  // .find() .findIndex() 作用是差不多的，只是後者會回傳index
  const friendIndex = friends.findIndex(friend => friend.id === id)
  // (２)或傳入的 id 在收藏清單中不存在，就結束這個函式，index這種東西如果找不到東西，就會回傳-1
  if (friendIndex === -1) return

  friends.splice(friendIndex, 1)

  localStorage.setItem('favoriteLists', JSON.stringify(friends))
